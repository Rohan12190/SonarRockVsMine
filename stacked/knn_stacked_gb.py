...
def find_optimal_k(X_train, X_test, y_train, y_test, k_range=range(1, 21)):
    ...
    plt.xticks(k_range)
    plt.grid(True)
    plt.savefig(f'{results_dir}/k_optimization_gb.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    plot_confusion_matrix(y_test, y_pred_knn, class_names, "Standalone KNN", f'{results_dir}/standalone_knn_gb_cm.png')

    # --- STACKED MODEL (KNN + GRADIENT BOOSTING) ---
    print("\n--- Evaluating Stacked Model (KNN + Gradient Boosting) ---")
    
    estimators = [
        ('knn', make_pipeline(StandardScaler(), KNeighborsClassifier(n_neighbors=optimal_k))),
        ('gb', GradientBoostingClassifier(n_estimators=100, random_state=42, learning_rate=0.1)) # No scaling needed for GB
    ]
    ...
    plot_confusion_matrix(y_test, y_pred_stacked, class_names, "Stacked (KNN+GB)", f'{results_dir}/stacked_gb_cm.png')
